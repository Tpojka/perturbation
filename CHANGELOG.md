...
- **Reading and writing a file both survive contention on Windows.** Python opens files without `FILE_SHARE_DELETE`, so a rename over a file someone has open is refused, and so is an open while a rename is in flight — and a refused read looked exactly like "no file", which meant the defaults. With one host per browser reading the settings twice a second, both became likely. The settings and the session files now go through one helper that retries either side for up to a second. Windows CI found both.
